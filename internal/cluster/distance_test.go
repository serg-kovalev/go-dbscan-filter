...
func (s *DistanceSuite) TestDistanceSphericalFast(c *C) {
	expected := 4.3026720164084415e-10
	actual1 := DistanceSphericalFast(&s.p1, &s.p2)
	actual2 := DistanceSphericalFast(&s.p2, &s.p1)
	// Use tolerance for floating point comparison
	c.Check(math.Abs(actual1-expected) < 1e-15, Equals, true)
	c.Check(math.Abs(actual2-expected) < 1e-15, Equals, true)
